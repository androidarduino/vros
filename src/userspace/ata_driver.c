...
static inline int syscall_ipc_recv(uint32_t port, void *msg)
{
    int ret;
    __asm__ volatile(
        "int $0x80"
...
// 处理块设备请求
static void handle_request(blkdev_request_t *req, uint32_t response_port)
{
    ...
    // 发送响应
    syscall_ipc_send(0, response_port, &resp, sizeof(resp));
}

// 用户空间 ATA 驱动主函数
void ata_driver_main(void)
{
    print("[ATA Driver] Starting user-space ATA driver...\n");

    // 1. 请求 I/O 端口访问权限
    print("[ATA Driver] Requesting I/O port access...\n");
    if (syscall_request_io_port(ATA_PRIMARY_IO, ATA_PRIMARY_IO + 7) != 0)
    {
        print("[ATA Driver] ERROR: Failed to get I/O port access\n");
        return;
    }
    if (syscall_request_io_port(ATA_PRIMARY_CTRL, ATA_PRIMARY_CTRL) != 0)
    {
        print("[ATA Driver] ERROR: Failed to get control port access\n");
        return;
    }
    print("[ATA Driver] I/O port access granted\n");

    // 2. 创建命名 IPC 端口
    print("[ATA Driver] Creating IPC port...\n");
    int port = syscall_ipc_create_named_port(BLKDEV_PORT_NAME);
    if (port < 0)
    {
        print("[ATA Driver] ERROR: Failed to create IPC port\n");
        return;
    }
    print("[ATA Driver] IPC port created\n");

    // 3. 注册 IRQ 处理器（可选）
    // syscall_register_irq_handler(ATA_PRIMARY_IRQ, port);

    print("[ATA Driver] Ready to serve block device requests\n");
    print("[ATA Driver] Port name: " BLKDEV_PORT_NAME "\n");

    // 4. 主循环：接收并处理请求
    while (1)
    {
        // 接收完整的 IPC 消息（必须与内核的 struct ipc_message 匹配）
        typedef struct
        {
            uint32_t sender_pid;  // offset 0
            uint32_t sender_port; // offset 4
            uint32_t type;        // offset 8
            uint32_t size;        // offset 12
            uint8_t data[256];    // offset 16
        } ipc_message_user;

        ipc_message_user msg;
        ...
        handle_request(req, msg.sender_port);
    }
}
